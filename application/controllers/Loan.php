...
class Loan extends CI_Controller {
	...
	public function index()
	{
		...
		$data['active_menu'] = 'data_loan';

		$search_query = $this->input->get('query');

		if ($search_query) {
			$data['loan'] = $this->LoanModel->search_title($search_query);
		} else {
			$data['loan'] = $this->LoanModel->get();
		}

		$data['books'] = $this->LibraryModel->get();
		$data['user'] = $this->session->userdata('username');
		$data['content'] = $this->load->view('loan_view', $data, true);
	...
	public function store()
	{
		...
		$this->form_validation->set_rules('borrowed_by', 'Dipinjam Oleh', 'required|trim');
		$this->form_validation->set_rules('borrow_date', 'Tanggal Pinjam', 'required|trim');
		$this->form_validation->set_rules('due_date', 'Tanggal Jatuh Tempo', 'required|trim');
		$this->form_validation->set_rules('return_date', 'Tanggal Kembali', 'required|trim');
